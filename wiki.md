...
|---|---|---|---|
| 1.4.0 | 2026-08-04 | Platform-loop pass | Ecosystem sweep for the null-`currentTeam` crash fixed in Dot.Mines commit `0cc4362`. Unlike Dot.Mines, **this platform has no `EnsureTeamContext` (or equivalent) middleware at all** — `routes/web.php`'s `/dashboard` and `/reports/{id}/download` closures, and every `api/v1/*` controller, run behind only `auth:sanctum`/`verified`, so `Auth::user()->currentTeam` was never guaranteed non-null; a user who belongs to no team (freshly registered before creating one, or removed from their last team) could hit any of these and get an unhandled null-property-access 500. Fixed at the two real entry points rather than in every call site: (1) `routes/web.php`'s `/dashboard` closure now redirects to `route('teams.create')` when `currentTeam` is null, before touching `$team->id` — this closes the loop for all 10 Livewire panels (`AlertsPanel`, `BusinessDnaPanel`, `CrossPlatformInsightPanel`, `EcosystemMapPanel`, `ExecutiveBriefingPanel`, `IntelligenceDashboard`, `KnowledgeGraphPanel`, `RecommendationsPanel`, `SavedReportsPanel`, `FeatureFlagsPanel`) too, since `resources/views/dashboard.blade.php` is the *only* place any of them are mounted (verified by grep — no other view references them) and a redirected request never reaches their `render()`/action methods; (2) `/reports/{id}/download` now `abort(403, 'No active team selected.')` when `currentTeam` is null, matching Dot.Mines' `ReportController` convention (no unscoped fallback — falling through would leak every team's data). For the API surface, centralised the duplicated `Auth::user()->currentTeam` lookup that was repeated verbatim across `ReportController` (3x), `IntelligenceController` (5x), `PlatformController::connect`, `MetricsController::aiUsage`, `SqlController` (2x), and `SavedReportController` (2x) into one `BaseApiController::currentTeam(): Team` helper (all V1 controllers already extend `BaseApiController`) that aborts 403 with the same message when null, then swapped every one of those 13 call sites to use it — removed the now-dead `Illuminate\Support\Facades\Auth` import from the 5 controllers that no longer reference `Auth::` directly. Also guarded `IngestController::receive()`'s `$team->id` inside its signature-mismatch log line (`$team?->id`) since that controller intentionally keeps its own `$user->currentTeam` lookup (webhook auth is per-Sanctum-token, not per-web-session, so it doesn't route through `currentTeam()`'s abort — a webhook caller with no team should still get its normal 401/404 responses, not a 403 that implies a *web* team-selection problem). Confirmed already-safe and left untouched: `HasTeamScope` (global scope, checks `Auth::check() && Auth::user()->currentTeam` before dereferencing), `DataSourcePolicy`/`CrossPlatformInsightPolicy` (`!== null`/`?->` guards throughout), `AppServiceProvider`'s Gate definitions (`isTeamOwnerOrAdmin()` returns false on null team), `SetLocale` middleware (`$user?->currentTeam`), `routes/channels.php` (`?->id`), and `FeatureFlagController::index()` (already `?->id`). Spot-checked the `HasTeamScope` trait rollout from Dot.Finance/Dot.Notify (`2f75bdb`/`e671436`): still intact on all 17 models with a `team_id` column; no gap found, nothing added. Added `tests/Feature/DashboardTest.php` (`test_authenticated_user_with_no_team_is_redirected_to_team_creation`, mirroring Dot.Mines' regression test exactly, plus a happy-path counterpart) and a matching `Tests\Feature\Api\ReportApiTest::test_user_with_no_team_gets_403_not_a_crash`. Ran the full suite against an isolated `dot_analytics_audit_test` Postgres database (dropped after the run): 494 tests, 481 passed, 6 failed, 7 skipped (1060 assertions) — the 6 failures are pre-existing and unrelated to this change (a `DataConnector.config` encrypted-cast-vs-`json`-column type mismatch in `ModelsTest`/`SqlApiTest`, and a `teams.locale` `varchar(10)` truncation in `SetLocaleTest`); none of the new regression tests are among the failures. Did not touch database schema, migrations, RLS, or queue/cache/search-index config, per scope. |
